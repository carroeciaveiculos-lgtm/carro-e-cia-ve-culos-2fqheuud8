...
async function handleFunctionCall(call: any) {
  ...
  if (call.name === 'consultar_estoque') {
    let query = supabase.from('veiculos').select('marca, modelo, preco_venda, ano_fabricacao, is_consignado').eq('status', 'disponivel')
...
Deno.serve(async (req) => {
  ...
  if (body.object === 'whatsapp_business_account') {
    ...
    if (!entry?.messages) {
      console.log("Evento recebido do WhatsApp, mas não é uma nova mensagem (ex: confirmação de leitura).");
    ...
    if (!lead) {
      console.error("Aviso: O processo foi interrompido porque o lead não pôde ser encontrado nem criado no banco.");
    ...
    if (historyError) {
      console.error("Erro crítico ao gravar histórico da mensagem do cliente no banco:", historyError);
    }
    ...
    if (aiRes.candidates?.[0]?.content?.parts?.[0]?.functionCall) {
      ...
      responseText = followUp.candidates?.[0]?.content?.parts?.[0]?.text || "Gostaria de agendar uma visita para conversarmos melhor?"
    } else {
  ...
  const { data: lead, error: portalInsertError } = await supabase.from('leads').insert({
    nome,
  ...
  if (lead) {
    console.log("Lead de portal criado com sucesso. Iniciando saudação por inteligência artificial...");
    ...
    const responseText = aiRes.candidates?.[0]?.content?.parts?.[0]?.text || `Olá ${nome}! Sou o Luiz, consultor digital da Carro e Cia. Vi que você tem interesse no ${veiculo}. Gostaria de agendar uma visita?`
