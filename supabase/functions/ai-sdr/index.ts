...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.action === 'init_conversation') {
      ...
      const initText = `Novo lead recebido do portal ${source}. O cliente se chama ${nome} e tem interesse no veículo: ${v}. Inicie a conversa se apresentando como Pedro, SDR da loja, e puxe assunto para qualificar a venda de forma amigável.`
      
      const aiRes = await runGemini([{ role: 'user', parts: [{ text: initText }] }])
      const responseText = aiRes.candidates?.[0]?.content?.parts?.[0]?.text || `Olá ${nome}! Sou o Pedro, consultor digital da Carro e Cia. Vi que você tem interesse no ${v}. Como posso te ajudar hoje?`
