    ...
    for (const veiculo of veiculos) {
        ...
        if (samePlacaVehicles && samePlacaVehicles.length > 0) {
          ...
          if (dupe.publicado_mercadolivre || dupe.publicado_webmotors || dupe.publicado_olx || dupe.publicado_icarros || dupe.publicado_napista) {
            ...
            logsToInsert.push({
              ...
              portal: 'geral',
              status: 'duplicado',
              payload_erro: { error: 'Veículo com mesma placa já publicado em outra plataforma', placa: veiculo.placa },
            })
          }
        }
      }
      ...
      results.forEach((res, idx) => {
        ...
        if (res.status === 'rejected') {
          ...
          logsToInsert.push({ veiculo_id: veiculo.id, portal, status: 'falha', payload_erro: { error: translated.message, action: translated.action, raw: res.reason?.message } })
        } else {
