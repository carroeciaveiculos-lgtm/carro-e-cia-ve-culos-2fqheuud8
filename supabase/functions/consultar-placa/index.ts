...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Atualizado endpoint para credits conforme a documentação
    ...
    try {
      ...
      throw new Error(`Falha de comunicação com a API Brasil (Status: ${res.status}). Verifique o token ou tente novamente mais tarde.`);
    }

    if (!res.ok || data?.error) {
      if (res.status === 401 || res.status === 403) {
        throw new Error('Token da API Brasil inválido ou expirado. Verifique as configurações de Secrets.');
      }
    ...
    // Tratamento atualizado da estrutura da API Brasil (data.resultados[0])
    let veiculoData: any = {};
    if (data?.data?.resultados && Array.isArray(data.data.resultados) && data.data.resultados.length > 0) {
      veiculoData = data.data.resultados[0];
    } else if (data?.dados) {
      // Fallback para a estrutura antiga caso a API retorne no formato anterior
    ...
    const result = {
      placa: veiculoData?.placa || placa,
      chassi: veiculoData?.chassi || '',
      renavam: veiculoData?.renavam || '',
      marca: veiculoData?.marca || '',
      modelo: veiculoData?.modelo || '',
      ano_fab: veiculoData?.anoFabricacao || veiculoData?.ano_fabricacao || '',
      ano_modelo: veiculoData?.anoModelo || veiculoData?.ano_modelo || '',
