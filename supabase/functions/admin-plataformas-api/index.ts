...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (method === 'GET' && (!slug || slug === 'plataformas')) {
    ...
    if (method === 'GET' && action === 'veiculos') {
      const page = body.page || 1
      const search = body.search || ''
      const pageSize = 24
      const offset = (page - 1) * pageSize
      let query = supabase.from('veiculos').select('id,marca,modelo,versao,ano_modelo,quilometragem,placa,preco_venda,fotos,publicado_mercadolivre,publicado_webmotors,publicado_olx,publicado_icarros,publicado_napista,status', { count: 'exact' }).eq('status', 'disponivel').order('created_at', { ascending: false }).range(offset, offset + pageSize - 1)
    ...
    if (method === 'POST' && action === 'sync' && segments[2] === 'forcar') {
      if (slug === 'mercadolivre') {
        await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/ml-sync`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')}` }, body: '{}' })
      }
      await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/sync-estoque`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')}` }, body: '{}' })
      await supabase.from('sync_log').insert({ plataforma_id: plataforma.id, acao: 'force_sync', status: 'success', mensagem: 'Sincronização forçada pelo admin' })
      ...
      return json({ success: true, message: 'Sincronização forçada iniciada' })
    }

    if (method === 'POST' && action === 'veiculos' && segments[2] === 'publicar') {
      ...
      await supabase.from('sync_log').insert({ plataforma_id: plataforma.id, veiculo_id: veiculoId, acao: publicar ? 'publish' : 'close', status: 'pending', mensagem: publicar ? 'Publicação solicitada' : 'Encerramento solicitado' })
