  ...
  try {
    ...
    if (payload?.event === 'document_signed') {
      ...
      if (fetchError || !contrato) {
        throw new Error(
          'Contrato não encontrado para o ID do Autentique: ' + autentique_document_id,
        )
      }
      ...
      return new Response(
        JSON.stringify({ success: true, message: 'Webhook Autentique processado com sucesso.' }),
        {
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'Evento Autentique recebido, mas não processado (não é document_signed).',
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      },
