...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { contrato_id, email_cliente, nome_cliente, pdf_url, proprietario_telefone, proprietario_cpf, numero_contrato } = body;
    ...
    const variables = {
        document: {
            ...
            file: pdf_url, 
            webhook_url: Deno.env.get('AUTENTIQUE_WEBHOOK_URL') || `${Deno.env.get('SUPABASE_URL')}/functions/v1/webhook-autentique`,
            auto_remind: true,
            expires_in: 7
        },
        signers: [
    ...
    if (!res.ok || resultado.errors) {
       ...
       await supabase.from('contratos_consignacao').update({ assinatura_link: mock_link, assinatura_id_externo: mock_id, assinatura_status: 'pendente' }).eq('id', contrato_id);
       
       return new Response(JSON.stringify({ success: true, mock: true, message: 'Contrato enviado (MOCK)', autentique_document_id: mock_id, signing_link: mock_link }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }});
    }
    ...
    await supabase.from('assinatura_historico').insert({
      ...
      detalhes: { email: email_cliente, link: link_assinatura_cliente, autentique_id: autentique_document_id }
    });
