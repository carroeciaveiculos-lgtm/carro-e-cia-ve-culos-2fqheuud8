...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authResult.success || !authResult.hashAutenticacao) {
      return new Response(JSON.stringify({ error: 'WM authentication failed', details: authResult.error }), {
    ...
    if (integracao) {
      await supabase
        .from('integracao_plataforma')
        .update({ credentials: { hashAutenticacao: hash }, ultima_sincronizacao: new Date().toISOString() })
        .eq('id', integracao.id)
    }
    ...
    for (const pub of pendingPubs) {
      ...
      if (!veiculo) {
        await supabase.from('estoque_publicacoes').update({ status: 'error', erro_msg: 'Vehicle not found' }).eq('id', pub.id)
      ...
      try {
        if (pub.status === 'pending_create' || pub.status === 'agendado') {
          ...
          const res = await callSOAP(xml, veiculo.categoria === 'Moto' ? 'IncluirMoto' : 'IncluirCarro')
          if (res.success && res.codigoAnuncio) {
            await supabase.from('estoque_publicacoes').update({
              status: 'publicado', post_id: res.codigoAnuncio, publicado_em: new Date().toISOString(), erro_msg: null,
            }).eq('id', pub.id)
            await supabase.from('veiculos').update({ publicado_webmotors: true }).eq('id', veiculo.id)
            results.push({ id: pub.id, status: 'created', codigoAnuncio: res.codigoAnuncio })
          } else {
            await supabase.from('estoque_publicacoes').update({ status: 'error', erro_msg: res.error }).eq('id', pub.id)
          ...
          if (res.success) {
            await supabase.from('estoque_publicacoes').update({ status: 'publicado', erro_msg: null }).eq('id', pub.id)
          ...
          if (res.success) {
            ...
            await supabase.from('veiculos').update({ publicado_webmotors: false }).eq('id', veiculo.id)
    ...
    if (wmPlataforma) {
      const logs = results.map((r) => ({
        ...
        acao: r.status === 'created' ? 'create' : r.status === 'updated' ? 'update' : r.status === 'closed' ? 'close' : 'error',
