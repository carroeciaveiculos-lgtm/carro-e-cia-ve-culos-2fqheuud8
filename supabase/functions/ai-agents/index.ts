...
const VEHICLE_FUNCTIONS = [
  ...
  },
  {
    ...
    parameters: { type: 'OBJECT', properties: { veiculo_id: { type: 'STRING' } }, required: ['veiculo_id'] },
  },
  {
    name: 'avaliar_troca',
    ...
    parameters: {
      ...
      properties: {
        ...
        ano: { type: 'INTEGER' }, quilometragem: { type: 'INTEGER' }, condicao: { type: 'STRING' },
      },
...
async function executeFunction(name: string, args: any, supabase: any): Promise<any> {
  ...
  if (name === 'buscar_veiculo_especifico') {
    const { data, error } = await supabase.from('veiculos').select('*').eq('id', args.veiculo_id).single()
    if (error) return { error: error.message }
    return { veiculo: data }
  }
  if (name === 'avaliar_troca') {
    const { data } = await supabase.from('veiculos').select('marca,modelo,ano_modelo,preco_venda,valor_fipe,quilometragem').ilike('marca', `%${args.marca}%`).ilike('modelo', `%${args.modelo}%`).limit(5)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fullPrompt = context ? `Contexto: ${JSON.stringify(context)}\n\nMensagem: ${message}` : message

    const gemini = new GeminiClient()
    const result = await gemini.generate(fullPrompt, { thinkingLevel: 'medium', temperature: 0.5, systemPrompt, functions: VEHICLE_FUNCTIONS })

    let functionResults: any[] = []
    if (result.functionCalls.length > 0) {
      functionResults = await Promise.all(result.functionCalls.map(async (fc) => ({ name: fc.name, result: await executeFunction(fc.name, fc.args, supabase) })))
      const followUp = await gemini.generate(`${fullPrompt}\n\nResultados:\n${JSON.stringify(functionResults, null, 2)}\n\nResponda ao usuario com base nessas informacoes.`, { thinkingLevel: 'medium', temperature: 0.5, systemPrompt })
      return new Response(JSON.stringify({ response: followUp.text, function_calls: result.functionCalls, function_results: functionResults }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    await supabase.from('logs_ia').insert({ usuario_id: userId, acao: `ai_agent_${agent_type}`, provider: 'google', modelo: 'gemini-1.5-flash', status: 'sucesso', tokens_input: result.tokenUsage.input, tokens_output: result.tokenUsage.output }).then(() => {}, () => {})
    return new Response(JSON.stringify({ response: result.text, function_calls: [], function_results: [] }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  } catch (err: any) {
