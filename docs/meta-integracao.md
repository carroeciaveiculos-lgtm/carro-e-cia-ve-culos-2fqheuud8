...
| **Instagram Stories implementado e testado ao vivo, Facebook Stories não (20/08/2026).** `publicar-social` lê `social_posts.content_type` (seletor "Formato" em `RedesSociais.tsx`, Central de Redes Sociais → Publicações) — `content_type='stories'` manda `media_type: 'STORIES'` pro Instagram (aceita foto ou vídeo, sem `caption` — Stories não exibe legenda via API). **Bug real achado no primeiro teste ao vivo**: publicar Stories de foto sem esperar o container terminar de processar devolvia `{success:true, media_id}` do Meta, mas o Story **não existia de verdade** (`GET /{media_id}` dava "does not exist"; `GET /{ig-id}/stories` vinha vazio) — sucesso falso do próprio Meta, documentado nos guias da API (publicar antes do container chegar em `FINISHED` pode devolver sucesso sem o conteúdo ir ao ar). Corrigido: `waitForInstagramMediaReady` (que já existia só pra vídeo) agora também roda pra Stories de foto. **Segundo teste ao vivo confirmou de verdade** (`media_id` apareceu em `GET /{ig-id}/stories`, não só como "Publicado" no banco). Facebook Stories usa endpoint totalmente diferente (`/photo_stories` ou `/video_stories`, não `/feed`/`/photos`) e provavelmente exige permissão extra do app ainda não confirmada — não implementado; se vier `stories` + Facebook marcado, `publicar-social` registra erro claro em vez de publicar errado no feed. Removida a duplicata: a aba "Redes Sociais" que existia dentro de `Marketing.tsx` (formulário próprio, `redes` em formato array incompatível, opção "google" nunca implementada) devia ter saído quando a Central de Redes Sociais foi criada em 14/08/2026 — nunca saiu. Removida agora; criar/agendar post social só existe em `/admin/central-social` | código em `publicar-social/index.ts` e `RedesSociais.tsx`, 20/08/2026 |
